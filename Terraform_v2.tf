...
resource "aws_internet_gateway" "INTERNET-GW-PROYECT" {
    vpc_id = aws_vpc.VPC-PROYECT.id  
}
...
resource "aws_route_table" "ROUTES-PUBLIC" {
    ...
    route {
        ...
        gateway_id = aws_internet_gateway.INTERNET-GW-PROYECT.id
    }
...
resource "aws_nat_gateway" "NAT-GW-PROYECT" {
...
resource "aws_route_table" "ROUTES-PRIVATE-A" {
      ...
      route {
          cidr_block = "0.0.0.0/0"
          nat_gateway_id = aws_nat_gateway.NAT-GW-PROYECT.id
      }
...
resource "aws_security_group" "SG-LoadBaPRO" {
...
resource "aws_lb" "LoadBaPRO-PROYECT" {
  name = "LoadBaPRO-PROYECT"
  internal = false
  ip_address_type = "ipv4"
  load_balancer_type = "application"
  subnets = [aws_subnet.SUBNET-PUBLIC.id, aws_subnet.SUBNET-PUBLIC-B.id]

  security_groups = [aws_security_group.SG-LoadBaPRO.id]
...
resource "aws_eip" "EIP-LINUX" {
    ...
    depends_on = [
      aws_internet_gateway.INTERNET-GW-PROYECT
    ]  
}
...
resource "aws_lb_listener" "LISTENER-WEB-PROYECT" {
  load_balancer_arn = aws_lb.LoadBaPRO-PROYECT.arn
  port = "80" 
  protocol = "HTTP"
...
resource "aws_lb_target_group" "TARGET-GROUP-PROYECT" {
  name = "TARGET-GROUP-PROYECT"
  port = 80
  protocol = "HTTP"
  vpc_id = aws_vpc.VPC-PROYECT.id  
}
